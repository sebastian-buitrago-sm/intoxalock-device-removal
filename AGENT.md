...
3. **`/spec-builder`** (skill) — TDD-build one slice from its `.feature`, following `docs/architecture`; logs each cycle to `progress/<feature>/tdd_<slice>.md`; gates on `make check`.
4. **`judge-iteration`** (subagent) — independently review the slice (coverage, TDD discipline, hexagon boundaries, gates) → `progress/<feature>/judge_<slice>.md`.
